...
Отдельно важно: большая часть того, что сейчас добирается из Google Sheets, в интегрируемом проекте, скорее всего, уже может быть восстановлена из собственной БД, истории цен, истории ставок, таблиц тестов и рекламной статистики. Поэтому Google Sheets здесь нужно воспринимать как **legacy fallback-источник**, а не как целевой источник данных. При переносе правильнее сначала проверить, какие поля уже доступны в новой БД, и по возможности заменить sheet-логику на собственные серверные данные и собственные расчеты.
...
Это место желательно не переносить буквально. В интегрируемом проекте большая часть этих данных обычно уже существует или может быть рассчитана из собственной БД:

- история цен по товару;
- рекламная статистика по окнам;
- история ставок;
- связь теста, товара и рекламной кампании;
- итоговые расчетные признаки по тесту.

Если эти сущности доступны на новой стороне, то `sheetPriceRows`, `sheetPriceDecisionRaw` и `sheetPriceDeviationCount` лучше строить заново из серверных данных нового проекта, а не оставлять зависимость от Google Sheets.
...
Практический приоритет здесь такой:

1. сначала проверить, какие данные уже есть в новой БД;
2. затем собрать эквиваленты price/snapshot verdict из этих данных;
3. только если каких-то данных реально нет, оставлять временный внешний fallback.

То есть для нового проекта Google Sheets не должны считаться обязательной частью архитектуры этого раздела.
